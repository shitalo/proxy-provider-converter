...
function RandUserAgent() {
    ...
    const uaLen = userAgents.length;

    const randomIndex = crypto.randomBytes(4).readUInt32LE() % uaLen;
    // const randomIndex = randIntN(uaLen);
...
function handleVShareLink(names, urlObj, scheme, proxy) {
    ...
    if (!urlObj.port) {
        return new Error('url.Port() is empty');
    }

    proxy['type'] = scheme;
    proxy['server'] = urlObj.hostname;
    proxy['port'] = urlObj.port;
    proxy['uuid'] = urlObj.username;
    proxy['udp'] = true;

    const tls = query.get('security')?.toLowerCase();
    ...
    let network = query.get('type')?.toLowerCase() || 'tcp';
    let fakeType = query.get('headerType')?.toLowerCase();
    ...
    switch (network) {
        ...
        case 'httpupgrade':
            ...
            if (earlyData) {
                ...
                if (isNaN(med)) {
                    return new Error('bad WebSocket max early data size');
                }
                switch (network) {
                    case 'ws':
...
// const scheme = 'vmess';
// const vmess = {};
// const line = 'vmess://44efe52b-e143-46b5-a9e7-aadbfd77eb9c@qv2ray.net:6939?type=ws&security=tls&host=qv2ray.net&path=%2Fsomewhere#VMessWebSocketTLS';
// const urlVMess = new url.URL(line);
//
// const err = handleVShareLink(names, urlVMess, scheme, vmess);
// console.log(names)
// console.log(scheme)
// console.log(vmess)
// console.log(line)
